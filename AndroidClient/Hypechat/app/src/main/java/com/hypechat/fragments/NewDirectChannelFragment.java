...
public class NewDirectChannelFragment extends Fragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        getActivity().setTitle("Nueva conversación");
    ...
    public static NewDirectChannelFragment newInstance(String organization) {
        NewDirectChannelFragment chatFragment = new NewDirectChannelFragment();
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        mChannelName = getView().findViewById(R.id.new_d_channel_et);
        mCreateNewChannel = getView().findViewById(R.id.new_d_channel_button);
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_new_direct_channel, container, false);
    }
    ...
    private void createNewChannel(){
        ...
        // perform the attempt.
        showProgress(true);
        final String userToStartConversationName = mChannelName.getText().toString();
        String organization_name = null;
        if (getArguments() != null) {
            organization_name = getArguments().getString("organization",null);
        }
        String currentUser = SessionPrefs.get(getContext()).getUsername();
        String channelName = currentUser.concat(",");
        channelName = channelName.concat(userToStartConversationName);
        if(!userToStartConversationName.isEmpty()){
            DirectChannelsPost channelCreateBody = new DirectChannelsPost(channelName);
            Call<Void> createOrganizationsCall = mHypechatRequest.createDirectChannel(organization_name, channelCreateBody);
            final String finalOrganizationName = organization_name;
            createOrganizationsCall.enqueue(new Callback<Void>() {
                @Override
                public void onResponse(Call<Void> call, Response<Void> response) {
                    processResponseNewChannelCreation(response, finalOrganizationName, userToStartConversationName);
                }
    ...
    private void processResponseNewChannelCreation(Response<Void> response, String organizationName, String channelName) {
        ...
        if (!response.isSuccessful()) {
            ...
            showNewChannelError(error);
                ...
                ((MainActivity) getActivity()).createNewDirectChannelFragment(organizationName,channelName);
        }
    }
    ...
    private void showProgress(final boolean show) {
        ...
        final TextView tv_start_conver = getView().findViewById(R.id.textView_start_conver);
        final TextInputLayout ti_new_channel = getView().findViewById(R.id.new_d_channel_ti_layout);
        final ProgressBar new_channel_pb = getView().findViewById(R.id.new_d_channel_progress);

        tv_start_conver.setVisibility(show ? View.VISIBLE : View.GONE);
        tv_start_conver.animate().setDuration(shortAnimTime).alpha(
                show ? 0 : 1).setListener(new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(Animator animation) {
                tv_start_conver.setVisibility(show ? View.GONE : View.VISIBLE);
            }
        });
